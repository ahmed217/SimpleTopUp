...
class Grameen(modem.Modem):
    def __init__(self, port,speed,pin,taskQ):

        super(Grameen, self).__init__(port,speed)
   	self.pin = pin
	self.taskQ = taskQ
	...
	d.query('update smsserver_out set successmsg=\''+successmsg+'\',transfer_success=\'success\' where id = ' + str(id))
        ...
        d.query('update wf_operator set balance = balance - '+str(amount)+' where op_code = \'017\'')
	...
	#count = 500
        while True:
	    if not self.modemOk('GRAMEEN'):
		self.logger.error('Please restart GRAMEEN MODEM')
		sys.stdout.write("\a")

		self.sendEmail('GRAMEEN')
